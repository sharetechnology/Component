...
//    __weak MailViewController * me = self;
    ...
    @weakify(self);
    MailData * mail = [self mailForIndexPath:[self.tableView indexPathForCell:cell]];
    
    if (direction == MGSwipeDirectionLeftToRight) {
        
        expansionSettings.fillOnTrigger = NO;
        expansionSettings.threshold = 2;
        return @[[MGSwipeButton buttonWithTitle:[self readButtonText:mail.read] backgroundColor:[UIColor colorWithRed:0 green:122/255.0 blue:1.0 alpha:1.0] padding:5 callback:^BOOL(MGSwipeTableCell *sender) {
            @strongify(self);
            MailData * mail = [self mailForIndexPath:[self.tableView indexPathForCell:sender]];
            mail.read = !mail.read;
            [self updateCellIndicactor:mail cell:(MailTableCell*)sender];
            [cell refreshContentView]; //needed to refresh cell contents while swipping
            
            //change button text
            [(UIButton*)[cell.leftButtons objectAtIndex:0] setTitle:[self readButtonText:mail.read] forState:UIControlStateNormal];
        ...
        MGSwipeButton * more = [MGSwipeButton buttonWithTitle:@"More" backgroundColor:[UIColor colorWithRed:200/255.0 green:200/255.0 blue:205/255.0 alpha:1.0] padding:padding callback:^BOOL(MGSwipeTableCell *sender) {
            @strongify(self);
            NSIndexPath * indexPath = [self.tableView indexPathForCell:sender];
            MailData * mail = [self mailForIndexPath:indexPath];
            MailTableCell * cell = (MailTableCell*) sender;
            [self showMailActions:mail callback:^(BOOL cancelled, BOOL deleted, NSInteger actionIndex) {
                if (cancelled) {
                    return;
                }
                if (deleted) {
                    ...
                    mail.read = !mail.read;
                    [(UIButton*)[cell.leftButtons objectAtIndex:0] setTitle:[self readButtonText:mail.read] forState:UIControlStateNormal];
                    [self updateCellIndicactor:mail cell:cell];
                    [cell refreshContentView]; //needed to refresh cell contents while swipping
                }
